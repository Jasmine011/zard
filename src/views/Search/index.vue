  ...
  <div class="search-container">
    ...
    <div class="main">
      <div class="py-container">
        ...
        <!--selector-->
        <search-selector
          @trademarkInfo="trademarkInfo($event)"
          @attrInfo="attrInfo"
        />
        <!--商品展示区-->
                ...
                <li :class="{ active: isOne }" @click="changeOrder('1')">
                  <a
                    >综合<span
                      v-show="isOne"
                      class="iconfont"
                      :class="{ 'icon-dingzhi': isUp, 'icon-xiazai-1': isDown }"
                    ></span
                  ></a>
                </li>
...
export default {
  ...
  methods: {
    ...
    //删除分类
    removeCategory() {
      //如果要发送的请求是非必传请求，且请求属性为空（为空时依然会发送这个属性，占宽带），可以把这个属性设置为undefined，这样在发送请求时就不会发送这个属性了。
    ...
    changeOrder(flag) {
      ...
      this.searchParams.order = `${flag}:${
        originOrder.split(":")[1] == "asc" ? "desc" : "asc"
      }`;
      this.getSearchData();
    },
    //点击页面操作，更新页面
  ...
  watch: {
    //监听路由，只要请求地址发生变化就说明请求发生了改变
    $route: {
      handler(newValue, oldValue) {
        //每一次请求把上一次请求的一二三级分类id清空，因为这三个id可能会同时存在
        this.searchParams.category1Id = undefined;
        this.searchParams.category2Id = undefined;
        this.searchParams.category3Id = undefined;
        //重新把请求数据传递到参数中
